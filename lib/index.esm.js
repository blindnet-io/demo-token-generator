...
function bin2B64Url(arr) {
    return str2B64Url(Array.from(arr).map(function (val) { return String.fromCharCode(val); }).join(''));
}
...
function signedToken(type, body, key) {
    return __awaiter(this, void 0, void 0, function () {
        var header, hb, hb_bytes, signature, b64signature;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    header = { 'alg': 'EdDSA', 'typ': type };
                    hb = str2B64Url(JSON.stringify(header)) + "." + str2B64Url(JSON.stringify(body));
                    hb_bytes = new TextEncoder().encode(hb);
                    return [4, nobleEd25519.sign(hb_bytes, key)];
                case 1:
                    signature = _a.sent();
                    b64signature = bin2B64Url(signature);
                    return [2, hb + "." + b64signature];
            }
        });
    });
}
var Token = (function () {
    ...
    Token.prototype.build = function () {
        var app = this.app ? { app: this.app.id } : {};
        var user = this.user ? { uid: this.user.id, gid: this.user.groupId } : {};
        var capture = this.capture ? { gid: this.capture.groupId, uids: this.capture.userIds ? this.capture.userIds.join() : undefined } : {};
        var access = this.access ? { did: this.access.dataId, gid: this.access.groupId } : {};
        var body = __assign(__assign(__assign(__assign(__assign({}, app), user), capture), access), { tid: 'test', exp: Math.floor(Date.now() / 1000) + (this.lifetime || 3600) });
        Object.keys(body).forEach(function (key) { return body[key] === undefined && delete body[key]; });
        var type = this.capture ? 'tjwt' : (this.access ? 'nejwt' : 'jwt');
        return signedToken(type, body, this.key);
    };
    return Token;
}());
var TokenBuilder = (function () {
    function TokenBuilder(token) {
        this.token = token;
    }
    TokenBuilder.init = function (key) {
        var keyBytes = typeof key === 'string' ? b64str2bin(key).slice(0, 32) : key;
        return new TokenBuilder(new Token(keyBytes));
    };
    TokenBuilder.prototype.build = function () {
        return this.token.build();
    };
    TokenBuilder.prototype.lifetime = function (time) {
        this.token.lifetime = time;
        return this;
    };
    TokenBuilder.prototype.forApp = function (appId) {
        var me = this;
        ...
        return {
            ...
            toCaptureData: {
                forGroup: function (groupId) {
                    me.token.capture = { groupId: groupId };
                    return me;
                },
                forUsers: function (userIds) {
                    me.token.capture = { userIds: userIds };
                    return me;
                }
            },
            toAccessData: {
                withId: function (dataId) {
                    me.token.access = { dataId: dataId };
                },
                inGroup: function (groupId) {
                    me.token.access = { groupId: groupId };
                }
            }
        };
    };
